...
export async function load() {
  const repo = 'reagent-systems/rigide';
  const repoUrl = `https://github.com/${repo}`;
  const res = await fetch('https://raw.githubusercontent.com/reagent-systems/rigide/main/README.md');
  if (!res.ok) {
    return { title: 'RigIDE', content: 'Failed to fetch README from GitHub.', link: repoUrl };
  }
